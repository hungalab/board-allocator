...
#--------------------------------------------------------------
def mcc(previsous_clique_number, rest_nodes, best_slot_num=float('inf')):
    ...
    for k in reversed(range(2, previsous_clique_number + 1)):
        if -(-len(rest_nodes) // k) >= tmp_best:
            break
        degree_constraints = {v: range(0, k, k - 1) if v in rest_nodes else 0 for v in GraphSet._vertices}
        num_edges = k * (k - 1) // 2
        cliques = GraphSet.graphs(degree_constraints=degree_constraints, num_edges=num_edges)
